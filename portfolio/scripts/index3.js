...
// Afficher dans la console tous les mots de passe, de 6 lettres, avec une boucle for
for (let i = 0; i < passwords.length; i++) {
...
// Afficher dans la console tous les mots de passe, commençant par la lettre "p", string[0] avec une boucle for
for (let i = 0; i < passwords.length; i++) {
    if (passwords[i][0] === "p") {
        console.log(passwords[i])
        }
...
const resPerson = getPerson(person2)
...
// Ecrire une fonction qui prend en paramètre un objet et qui retourne la phrase : "Pierre Dupont n'a pas le droit de boire de l'alcool aux USA" 
// Paul Doazan a le droit de boire de l'alcool aux USA. Age légal de consommation alcool aux USA : 21

const resAlcool1 = getAlcool(person1)
const resAlcool2 = getAlcool(person2)

console.log(resAlcool1, resAlcool2)

function getAlcool(person) {
    if (person.age >= 21) {
        return(`${person.firstname} ${person.lastname} a le droit de boire de l'alcool aux USA. Age légal de consommation alcool aux USA : 21`)
    } else {
        return(`${person.firstname} ${person.lastname} n'a pas le droit de boire de l'alcool aux USA`)
    }
}